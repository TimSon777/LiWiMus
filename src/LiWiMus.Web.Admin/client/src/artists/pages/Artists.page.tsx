...
export default function ArtistsPage() {
    ...
    const columns: GridColDef[] = [
        ...
        },*/
        {
            ...
            renderCell: (params) => {
                const onClick = (e: any) => {
                    e.stopPropagation();
                    const path = "/admin/artists/" + params.getValue(params.id, "id");
                    navigate(path);
                };
    ...
    const filterModelInitialState = columnsOptions.map((column, index) => ({
        id: index, filterColumn: column,
        filterOperator: '',
        filterValue: ''
    }));
    const [filterModel, setFilterModel] = useState<FilterModel>(filterModelInitialState);

    const onOperatorChange = (e: any) => {
        setFilterModel(prevState => (
            prevState.map(el => (el.id.toString() === e.target.id ? {
                ...el, filterOperator: e.target.value
            } : el))
        ))
    }
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const testColumn: keyof Artist = sortModel[0].field as any
        ...
        <div>
            ...
            <div>
                <Button aria-describedby={id} onClick={handleClick} variant="contained"
                        sx={{borderRadius: "20px", marginBottom: "10px", px: 4}} startIcon={<FilterListIcon/>}>
                    Filters </Button>
                <FiltersPopover id={id} open={open} anchorEl={anchorEl} onClose={handleClose}
                                columnsOptions={columnsOptions} operatorsOptions={operatorsOptions}
                                onOperatorChange={onOperatorChange} onFilterValueChange={onFilterValueChange}
            ...
            <Fab color="primary" sx={{ position: 'fixed', bottom: 50, right: 50}} aria-label="add" href="/admin/artists/create">
                <AddIcon/>
            </Fab>
        </div>
    );
